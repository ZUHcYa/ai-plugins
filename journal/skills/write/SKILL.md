---
name: "write"
description: "Add a journal entry — interactive guided mode with topic and task prompts"
---
...
Add entries to today's journal file with guided prompts. Supports two modes:
- **Without args:** Interactive — Claude asks for topic and content (and tasks if inline mode)
- **With args:** One-shot — directly append to today's file

Use `/journal <text>` instead if you want to write without any prompts.

## When to Use

- Writing a thoughtful daily reflection
- Logging a learning or insight with optional topic tagging
- Capturing a note with tasks attached (inline task management)
- When you want to be guided through topic → content → tasks step by step
...
```
User: /journal:write Quick standup note: blocked on API integration, waiting for credentials
Claude: Entry added to entries/2026-02-21.md
...
```
User: /journal:write #standup Blocked on API integration, waiting for credentials
Claude: Entry added to entries/2026-02-21.md
...
- For quick writes without prompts, use `/journal <text>` instead
